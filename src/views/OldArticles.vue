<template>
  <div class="container">
    <!-- loading -->
    <template v-if="isItemsLoading"> 取得 iM 文章列表中... </template>

    <!-- loaded -->
    <template v-else>
      <div class="d-flex">
        <button @click.prevent="setPage(currPage - 1)">上一頁</button>
        <button class="ml-auto" @click.prevent="setPage(currPage + 1)">
          下一頁
        </button>
      </div>
...
export default {
  ...
  computed: {
    ...
    ...mapState("articles", ["items", "isItemsLoading", "isItemsLoaded"]),
