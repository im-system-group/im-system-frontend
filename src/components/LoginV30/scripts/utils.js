...
/** 當用戶按下按鍵或點選頁面 */
export function whenUserPressKeyOrClickPage() {
...
/** 當用戶填寫了表單且按下Enter */
export function whenUserFilledOutFormAndPressEnter() {
    const {
        accountField,
        passwordField,
    } = elements;
    ...
    return new Promise(callbackWhenUserFilledOutFormAndPressEnter => {
        ...
        const handleUserKeyPress = (event) => {
            ...
            if (isUserFilledOutForm && isUserPressEnter) {
                callbackWhenUserFilledOutFormAndPressEnter({ account, password });
...
/** 格式時間 */
export function formatToTimeText(time) {
    return [
        time.getHours(),
        time.getMinutes(),
        time.getSeconds(),
